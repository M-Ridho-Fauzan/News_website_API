...
<form action="{{ $action }}" method="{{ $method }}"
    {{ $attributes->merge(['class' => 'flex flex-col items-center justify-between lg:w-3/4 w-full gap-6 lg:flex-row lg:gap-8']) }}>
    <x-preserve-query />

    <input type="text" name="kategori" value="{{ request('kategori') }}" hidden>
    {{-- {{ buildQueryUrl('all-post', ['kategori' => $item['id_kategori']]) }} --}}
